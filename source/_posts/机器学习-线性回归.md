...
> 比如 $N < n$（样本数量小于特征种类的数量），根据 $X$ 的秩小于等于 $N,n$ 中的最小值，即小于等于 $N$（矩阵的秩一定小于等于矩阵的行数和列数）； 而矩阵 $X^TX$ 是 $n\times n$ 大小的，它的秩一定小于等于 $n$ ，因此不是满秩矩阵。
...
<center> $J({\vec\theta}) = {1\over2}\sum^m_{i=1}(h_{\vec\theta}(x^{(i)}) - y^{(i)}) + \lambda\sum^n_{j=1}{|\theta_j|}$ </center></br>



### 岭回归

<center> $J({\vec\theta}) = {1\over2}\sum^m_{i=1}(h_{\vec\theta}(x^{(i)}) - y^{(i)}) + \lambda\sum^n_{j=1}{\theta_j^2}$ </center></br>

### Elastic Net 回归

<center>$J(\theta) = {1\over 2}\sum_{i=1}^m (h_{\theta}(x^{(i)}) - y^{(i)}) + \lambda p\sum_{j=1}^b||\theta_j||_1  + {\lambda(1-p) \over 2} \sum_{j=1}^n||\theta_j||_2$</center></br>
