...
> 设 $(X，Ｙ)$ 是二维随机变量，对于任意实数 $x, y$, 二元函数：$F(x,y)=P\{ (X\leq x) \and (Y\leq y)\}=P\{ X\leq x , Y\leq y\}$称为二维随机变量 $(X, Y)$ 的分布函数，或随机变量 $X, Y$ 的联合分布函数．
...
概率密度函数 $f_{XY}(x,y)$ 的值总是非负的，但是可能会大于 $1$．尽管如此，在某些情况下 $\int _{-\infty}^{\infty} \int _{-\infty}^\infty f_{XY}(x,y) = 1$．
